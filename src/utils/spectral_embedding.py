"""
Copied from https://github.com/scikit-learn/scikit-learn/blob/7f9bad99d6e0a3e8ddf92a7e5561245224dab102/sklearn/manifold/_spectral_embedding.py#L143
Also see https://github.com/scikit-learn/scikit-learn/blob/7f9bad99d/sklearn/cluster/_spectral.py#L192
"""
import warnings
...
def spectral_embedding(
    ...
    * :doi:`"Toward the Optimal Preconditioned Eigensolver: Locally Optimal
    ...
    laplacian, dd = csgraph.laplacian(
    ...
    if (
        ...
        try:
            ...
            # laplacian = cupyx.scipy.sparse.csr_matrix(laplacian)
            # eigenvalues, diffusion_map = cupyx.scipy.sparse.linalg.eigsh(
            #     laplacian, k=n_components, which="LM", tol=tol
            # )
            # eigenvalues = cp.asnumpy(eigenvalues)
            # diffusion_map = cp.asnumpy(diffusion_map)
        ...
        # laplacian = cupyx.scipy.sparse.csr_matrix(laplacian)
        # X = cp.asarray(X)
        # M = cupyx.scipy.sparse.linalg.aslinearoperator(M)
        # eigenvalues, diffusion_map = cupyx.scipy.sparse.linalg.lobpcg(
        #     laplacian, X, M=M, tol=tol, largest=False)
        # eigenvalues = cp.asnumpy(eigenvalues)
        # diffusion_map = cp.asnumpy(diffusion_map)
    ...
    if eigen_solver == "lobpcg":
        ...
        if n_nodes < 5 * n_components + 1:
            ...
            try:
                laplacian_cu = cupyx.scipy.sparse.csr_matrix(laplacian)
                X_cu = cp.asarray(X)
                eigenvalues, diffusion_map = cupyx.scipy.sparse.linalg.lobpcg(
                    laplacian_cu, X_cu, tol=tol, largest=False, maxiter=2000
                )
                eigenvalues = cp.asnumpy(eigenvalues)
                diffusion_map = cp.asnumpy(diffusion_map)
            except Exception as e:
                print("GPU lobpcg failed, falling back to CPU version, error: ", e)
                eigenvalues, diffusion_map = lobpcg(
    ...
    if drop_first:
        return eigenvalues[1:n_components], embedding[1:n_components].T
    else:
        return eigenvalues[:n_components], embedding[:n_components].T
