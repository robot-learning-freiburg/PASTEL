...
class Cityscapes(Dataset):
    ...
    def __init__(
        ...
        elif self.mode in ["train_extra", "video"]:  # no labels available
        ...
        if self.mode == "train" and load_test_image:
            ...
            self.path_base = Path(__file__).absolute().parent.parent / "test" / "cityscapes"
            self.test_frame_paths = self._get_frames()
    ...
    def __getitem__(
        ...
        add_random_other_sample : bool
            If True, add a random other sample to the batch.
        Returns
        -------
        output : dict
...
class CityscapesDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        if stage == "fit" or stage is None:
            self.cityscapes_train = Cityscapes(self.train_set, self.cfg_dataset,
